...
class Valentines(commands.Cog):
    ...
    async def first_announcement(self):
        ...
        for server in servers:
            try:
                ...
                embed.add_field(name="What do I do with these items?", value="Once you obtain an item, you then have to give it to someone else.\nYou can do so using the `/valentines give` command.\nYou can't give items to yourself, or to bots.", inline=False)
                embed.add_field(name="How do I see what items I have?", value="You can use `/valentines inventory` to see what items you can give to others, as well as what items you have sent to others.", inline=False)
                embed.add_field(name="What happens when the event is over?", value="Each item sent grants you a certain amount of points.\nThe points are as follows:\n{}\nAfter the event is over, the person with the most amount of points will win!".format("\n".join(f"- {item['emote']} **{item['name']}**: {item['points']} points" for item in self.items)), inline=False)
    ...
    async def last_announcement(self):
        ...
        for server in servers:
            ...
            if len(winners) == 0:
                embed.description += "\nNo one won this year, as no person sent any items.\n\nHope you had a wonderful Valentine's Day! " + random.choice([e for e in "🧡💗💖💙💓💚💝💜💛🤍"])
            elif len(winners) == 1:
                w = winners[0]
                embed.description += "The winner of the Valentine's Day Event is <@{}>, who sent **{}**!".format(
                    winners[0],
    ...
    @app_commands.describe(message="An optional message you can send to the user")
    async def give(self, inter: discord.Interaction, item: app_commands.Choice[str], user: discord.Member, message: str=None):
        ...
        self.vdb.update_one({"_id": inter.guild_id}, {"$set": {str(inter.user.id) + "." + item["name"] + "-r": tempdata[item["name"] + "-r"] + 1}})
        embed = functions.embed("Item Received!", color=COLOR)

        embed.description = "*" + random.choice(item["poems"]) + "*\n\nYou were given `" + item["name"] + "` by " + inter.user.mention + "!\n" + (('```' + message + '```\n') if message else '') + "Use `/valentines inventory` to see if you have anything to send.\nHappy Valentine's Day! "
